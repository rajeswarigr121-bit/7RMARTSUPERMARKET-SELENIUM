...
public class FileUploadutility {
	...
	public void robotclassforfileupload(WebElement element, String path) throws AWTException {
		StringSelection selection = new StringSelection(path); // StringSelection is the class user when we use robot
																// class
		Toolkit.getDefaultToolkit().getSystemClipboard().setContents(selection, null); // We use this step to copy the
																						// path in clipboard Toolkit is
																						// class
																						// //getDefaultToolkit().getSystemClipboard().setContents()
																						// - all these are methods used
																						// in Tookit
