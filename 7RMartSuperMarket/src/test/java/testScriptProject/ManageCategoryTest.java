...
public class ManageCategoryTest extends Basetest {
	...
	@Test(description = "Verify the Manage Category Page")
	public void enterUsernameAndPassword() throws IOException {
		...
		category.clickOnNewButton().enterCategory("Screenshot").clickOnDiscount().uploadChooseFileButton()
				.clickOnShowTopmenu().clickOnSavebutton();
		 ...
		 * category.clickOnmoreinfo(); category.clickOnNewButton();
		 * category.enterCategory("vegetables"); category.clickOnDiscount();
		 * category.uploadChooseFileButton();
		 * category.uploadChooseFile(constants.IMAGEPATH);
		 * category.clickOnShowTopmenu(); category.clickOnSavebutton();
		 */
	}
}